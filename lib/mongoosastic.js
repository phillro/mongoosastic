    ...
    esClient.search(query, function(err, results, res){
      if(err){
        ...
        if (alwaysHydrate || options.hydrate) {
          var hydrateOptions = options.hydrateOptions||{}
          hydrate(results, model,hydrateOptions, cb);
        }else{
          cb(null, res);
        }
      }
...
function hydrate(results, model,hydrateOptions, cb){
  var ids = results.hits.map(function(a){
    return a._id;
  });
  model.find({_id:{$in:ids}},{},hydrateOptions, function(err, docs){
    if(err){
      return cb(err);
    }else{
      results.hits = docs;
