                            ...
                            async.waterfall([
                                ...
                                function createShortUrl(sourceContent, articleData, article, createShortUrlCallback) {
                                    services.shortenerService.generate(url, {data:{articleId:article._id}}, function (error, shortUrl) {
                                        models.Article.update({_id:article._id},{$set:{ma_short_url_hash:shortUrl.hash}},{multi:true},function(upError,upResult){
                                            createShortUrlCallback(error, sourceContent, articleData,article)
                                        })
